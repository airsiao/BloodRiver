package com.ourgame.mahjong.bloodriver.lobby.model
{
	 ...
	 * @author SiaoLeon
	 */
	public class BaseSocketModel extends Model
	{
		// -------------------------------------------------------------------------------------------------------- 静态常量
		 ...
		 * 构造函数
		 */
		public function BaseSocketModel()
		{
			super();
		}
		...
		{
			this.notify(SocketMethod.CONNECT_SUCCESS(BaseSocketModel));
		}
		
		public function onError(event:SocketEvent):void
		{
			this.notify(SocketMethod.CONNECT_ERROR(BaseSocketModel));
		}
		
		public function onClosed(event:SocketEvent):void
		{
			this.notify(SocketMethod.CLOSED(BaseSocketModel));
		}
		
		public function onDisconnect(event:SocketEvent):void
		{
			this.notify(SocketMethod.DISCONECT(BaseSocketModel));
		}
		
		public function onFlush(event:SocketEvent):void
		{
			this.notify(SocketMethod.FLUSH(BaseSocketModel), event.data);
		}
		
		public function onRecive(event:SocketEvent):void
		{
			this.notify(SocketMethod.RECIVED(BaseSocketModel), event.data);
		}
